...
def get_neighbors(w):
    ...
    """
    Return all words in word_set that have 1 and one 1 letter different
    """
    neighbors = []
...
def find_ladders(begin_word, end_word):
    """
    Find a word transformation netween begin and end word
    """
    q = Queue()
    ...
    while q.size() > 0:
        ...
        w = path[-1]
        #Check if its target word, if so return ath
        if w== end_word:
            return path
